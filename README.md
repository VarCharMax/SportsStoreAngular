...
The author's implementation of the Product Editor is dreadful, and an object lesson in how not to implement such a feature. It's also incompatible with my async reiplementation of hte repository model, as it can't write directly to the repository, as the original design required. However, rather than completely re-implementing it, I decided to treat it as I would a legacy code issue, and come up with a fix that does not require substantially rewriting the codebase. So the code still isn't that great, but it's compatible with the code as supplied.
